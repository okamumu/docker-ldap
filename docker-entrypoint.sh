...
# replace variables in organization configuration
ORG_CONF="/etc/openldap/organization.ldif"
sed -i "s~%SUFFIX%~$SUFFIX~g" "$ORG_CONF"
sed -i "s~%ORGANIZATION_NAME%~$ORGANIZATION_NAME~g" "$ORG_CONF"

# add organization and users to ldap (order is important)
slapadd -l "$ORG_CONF"

# add any scripts in ldif
for l in $LDIF_DIR/*; do
  case "$l" in
